...
void pushTruthValue(int value[MAX], int num, int len);
...
char* parse(struct node* root, char* exp){
    ...
    printf("0\n");
    for(int i=0; i<cntArr(value); i++) printf("%d ", value[i]);
...
void preOrderPrint(struct node *root){
    ...
    if(root->data != 0 && root->data != 1 && root->data != -1){
        ...
        printf("%d ", val);
        int len = cntArr(value);
        pushTruthValue(value, val, len);
    }
